    ...
    return (
      ...
      >
        ...
        <div className="space-y-2">
          <h2 className="text-3xl font-black text-gray-900 uppercase">{t('আপনার কার্ট খালি', 'Your Cart is Empty')}</h2>
          <p className="text-gray-500">{t('মনে হচ্ছে আপনি এখনও আপনার কার্টে কিছু যোগ করেননি।', "Looks like you haven't added anything to your cart yet.")}</p>
        </div>
        <Link 
          to="/" 
  ...
  return (
    ...
    >
      <h1 className="text-3xl font-black uppercase tracking-tight mb-8">
        {t('শপিং কার্ট', 'Shopping Cart')} ({cart.length})
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="hidden md:grid grid-cols-6 gap-4 px-6 py-4 bg-white rounded-t-xl border border-gray-100 text-[10px] font-black uppercase text-gray-400 tracking-widest">
            <span className="col-span-3">{t('পণ্যের বিবরণ', 'Product Detail')}</span>
            <span className="text-center">{t('মূল্য', 'Price')}</span>
            <span className="text-center">{t('পরিমাণ', 'Quantity')}</span>
            <span className="text-right">{t('মোট মূল্য', 'Subtotal')}</span>
          </div>
          ...
          <div className="space-y-3">
            {cart.map(item => (
              <motion.div 
                ...
                className="bg-white p-4 md:px-6 md:py-8 border border-gray-100 shadow-sm relative group"
                style={{ borderRadius: `${l?.borderRadius || 12}px` }}
              >
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-center">
                  <div className="col-span-3 flex gap-4 items-center">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        <Trash2 size={12} /> {t('মুছে ফেলুন', 'Remove')}
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="text-right flex md:block items-baseline justify-between mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-50">
                    <span className="md:hidden text-[10px] font-bold uppercase text-gray-400">{t('মোট', 'Total')}</span>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-28" style={{ borderRadius: `${l?.borderRadius || 16}px` }}>
            ...
            <div className="space-y-4 mb-8">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('মোট পণ্য', 'Subtotal')} ({cart.length} {t('টি আইটেম', 'items')})</span>
                <span className="font-bold text-gray-900">{formatPrice(subtotal)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('শিপিং চার্জ', 'Shipping Fee')}</span>
                <span className={cn("font-bold", shipping === 0 ? "text-green-500" : "text-gray-900")}>
                    {shipping === 0 ? t('বিনামূল্যে', 'FREE') : formatPrice(shipping)}
                </span>
              </div>
              {shipping > 0 && (
                  <p className="text-[10px] text-gray-400 font-medium">{t('আর', 'Add')} {formatPrice(5000 - subtotal)} {t('টাকার পণ্য কিনলেই শিপিং চার্জ ফ্রি!', 'more for FREE shipping!')}</p>
              )}
              <div className="pt-4 border-t border-gray-100 flex justify-between items-baseline">
                <span className="text-base font-black uppercase">{t('সর্বমোট', 'Estimated Total')}</span>
                <span className="text-2xl font-black" style={{ color: c?.primary || settings.primaryColor }}>
                  {formatPrice(total)}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
                <button 
                  ...
                  className="w-full py-4 text-white font-black uppercase tracking-widest shadow-lg flex items-center justify-center gap-2"
                  style={{ 
                ...
                >
                  {t('চেকাউট করুন', 'Proceed to Checkout')} <ArrowRight size={18} />
                </button>
                <Link 
                  to="/" 
                ...
                >
                  {t('মার্কেটে ফিরে জান', 'Continue Shopping')}
                </Link>
            </div>
